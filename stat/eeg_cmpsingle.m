function out = eeg_cmpsingle(x,y,varargin)

%EEG_CMPSINGLE compare single trial ERPS (spatial embedding)
%
% function [out] = cmpsingle(X,Y,roi,time,samples,alpha,eps,norm)
%
...
%
% The phase space is constructed using spatial 
% embedding using the given ROI for a given time 
% period. 
%
...
if ~isempty(varargin{5}), epsFactor = varargin{5};else epsFactor = .05;end
if ~isempty(varargin{6}), norm = varargin{6};else norm = 'max';end
...
for i=1:size(allCombos,1)
	...
	dataX = double(x(roi,timeWindow,allCombos(i,1)))';
	dataY = double(y(roi,timeWindow,allCombos(i,2)))';

	%eps estimation and RP for X
	eps = pss(dataX,norm) *epsFactor;
	rpX = crp2(dataX,[],[],[],[],eps,norm,'sil');

	%eps estimation and RP for Y
	eps = pss(dataY,norm) *epsFactor;
	rpY = crp2(dataY,[],[],[],[],eps,norm,'sil');
