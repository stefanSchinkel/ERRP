function idx = rejectEEGlab(inFile,varargin)
...
%  	%% reject abnormal values by threshhold
	[EEG] = pop_eegthresh( EEG,1,1:EEG.nbchan, -40, 40,-.6,.2,0,0);

	% reject abnormal by kurtosis
	[EEG] = pop_rejkurt( EEG, 1, 1:EEG.nbchan,5, 5, 0, 0, 0);
	
%	% reject due to linear trend
	EEG = pop_rejtrend( EEG, 1, 1:EEG.nbchan, 50, 50, .3, 0, 0,0);

%	% reject due to improabable values
%	EEG = pop_jointprob(EEG, 1,1:EEG.nbchan,5, 5, 0, 0,0);
	eval([theVar ' = EEG.data;'])
	ALLEEG = pop_delset(ALLEEG,[1]);
	idx = unique([find(EEG.reject.rejkurt) find(EEG.reject.rejthresh)  find(EEG.reject.rejconst)])
