...
% along with this program.  If not, see <http://www.gnu.org/licenses/>.
	...
	%convert symbols to 1,2...N
	symbols = myUnique(ops);
	for iSym = 1:numel(symbols)
		ops(ops == symbols(iSym) ) = iSym;
	end


	% pre allocate output
	nw = double( zeros( nChan,nChan, nEpoch - (dim-1)*tau ));
	
	% construct NWs
	for iEpoch = 1:size(ops,2)
		X = ops(:,iEpoch);
		Y = X';
		t = ( X(:,ones(1,nChan)) == Y(ones(1,nChan),:) )' .* X(:,ones(1,nChan));

		nw(:,:,iEpoch) = t;		
	end % epoch loop
end %opnw2

% simple helper function to emulate unique()
% which will change in future releases
