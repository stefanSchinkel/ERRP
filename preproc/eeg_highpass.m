function EEG = eeg_highpass(EEG,cutoff,sampRate,varargin)

%EEG_HIGHPASS high-pass filter an EEG set
%
% function [EEG] = eeg_lowpass(EEG,cutoff,sampRate[,order])
%
% Highpass filter an EEG set at the given cutoff frequency using 
...
%
% Output:
%  EEG      = high-pass filtered EEG set
%
% requires: highPass.m (ERRP/helper)
%
% see also: ERRP
%
% References:
...
% assign parameters
if ~isempty(varargin{1}), order = varargin{1};else order = 2;end
...
if ndims(EEG) == 2,
	...
	for i = 1:size(EEG,1) %chans
			EEG(i,:) = highPass(EEG(i,:),cutoff,sampRate,order);
	end
else
	...
	for i = 1:size(EEG,1) %chans
		for j = 1:size(EEG,3) %trials
			EEG(i,:,j) = highPass(EEG(i,:,j),cutoff,sampRate,order);
		end
	end
